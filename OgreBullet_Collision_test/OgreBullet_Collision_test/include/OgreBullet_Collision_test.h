 ...
 public:
 	...
 	virtual bool processUnbufferedKeyInput(const FrameEvent& evt)
 	{
 		...
 		if(mKeyboard->isKeyDown(OIS::KC_B) && mTimeUntilNextToggle <=0)
 		{
			...
			createBox(Vector3(0,20,0));
			createBox(Vector3(-15,20,0));
			
 		}
 		return ret;
 	}

	virtual void createBox(Vector3& v){
		Vector3 size = Vector3::ZERO;	// size of the box
 			// starting position of the box
 			//Vector3 position = (mCamera->getDerivedPosition() + mCamera->getDerivedDirection().normalisedCopy() * 20);
			Vector3 position = (v);
